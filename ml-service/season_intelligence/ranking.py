...
from season_intelligence.contracts import Candidate, RankedCandidate
from season_intelligence.model import attribute_similarity
...
class CandidateReranker:
    ...
    def rank(
        ...
        for candidate in candidates:
            ...
            left_price = max(float(product.get("mrp") or 1), 1)
            right_price = max(float(candidate.item.get("mrp") or 1), 1)
            feature_rows.append({
                "vector_similarity": candidate.vector_similarity,
                "attribute_similarity": attribute,
                "price_similarity": math.exp(-abs(math.log(left_price / right_price)) / 0.35),
        ...
        if self.model is not None and len(matrix):
            ...
            weights = np.asarray([0.50, 0.28, 0.08, 0.07, 0.07], dtype=np.float64)
